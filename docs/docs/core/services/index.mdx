...
These services are simply classes that implement `TegralService` and that get put in a [services-enabled](/core/di/extensions/services.md) [Tegral DI](/core/di/index.mdx) environment

In order to make using services easier, Tegral Services also provides a feature (`ServicesFeature`) that automatically sets up the [Tegral DI services extension](/core/di/extensions/services.md) for you when enabled.
