...
import step1HelloWorld from '!!raw-loader!./step-1-settings.gradle';
import step1HelloWorldKts from '!!raw-loader!./step-1-settings-kt.gradle.kts';
...
<CodeBlock language="groovy">{step1HelloWorld.replace("%VERSION%", version())}</CodeBlock>
...
<CodeBlock language="groovy">{step1HelloWorldKts.replace("%VERSION%", version())}</CodeBlock>
