...
Fundef support must be enabled in Tegral Web using the following in your `tegral` block:
...
Ktor modules and controllers can also be defined using [fundefs](../../core/di/fundefs.md). This is a shorter syntax that is more convenient for controllers and modules that do not require complex logic.

Fundefs that have Ktor's `Application` or `Routing` class as a receiver are installed as Ktor modules and Ktor controllers respectively. For example, this:
