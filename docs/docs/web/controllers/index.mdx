...
You do not need to add this class yourself when using AppDSL/AppDefaults. A [default application class](/web/appdefaults/index.mdx) is created for you with sane defaults and uses the [web configuration](/web/config/index.mdx) to configure the port and host.
...
Do not forget the `get()` after `val settings`, otherwise you will be performing an [unsafe injection](/core/di/testing/checks.md#safe-injection-only) if you use DI components when building your settings object.

:::

### KtorModule

A Ktor module (not to be confused with a [Tegral DI module](/core/di/modules.md)) is a class that inherits from `KtorModule`. This is a module in the Ktor sense: an extension function on top of `Application` that adds features, routes, etc.
